...
import re, os, sys, time, random, json, hashlib
...
def delay(night, a, b):
    """Спит от a до b секунд. В ночном режиме ×1.3."""
    m = 1.3 if night else 1
...
REGISTRY_FILE = "BD/registry.json"
...
def append_history(city, sphere, count, _filename):
    rows = read_history()
    rows.append({
        "city": city,
        "sphere": sphere,
...
def make_dedup_key(phones, address=None):
    """Ключ дедупликации — только номер телефона."""
    return phones.strip()
...
def registry_dedup_set(registry):
    """Множество dedup_key всех записей в реестре."""
    return {rec["dedup_key"] for rec in registry["records"].values()}


def filter_new_records(records, registry, dedup_set):
    """
    Фильтрует записи: пропускает только те, чей номер телефона
    ещё не встречался в реестре. Добавляет новую запись в registry['records'].
    Возвращает новые записи (уже добавленные в реестр).
    """
    now = datetime.now().strftime("%d.%m.%Y %H:%M")
    result = []
    for rec in records:
        dk = make_dedup_key(rec["phones"])
        if dk in dedup_set:
            continue
        dedup_set.add(dk)
        registry["records"][dk] = {
...
def collect_city_sphere(page, city, sphere, limit, seen, night, registry, dedup_set):
    sources = [
    ...
    while len(all_records) < limit:
        ...
        for src_name, src_func in sources:
            ...
            try:
                ...
                new_all = filter_new_records(records, registry, dedup_set)
                new_this_cycle += len(new_all)
                for rec in new_all:
                    all_records.append(rec)
                skipped = len(records) - len(new_all)
                suffix = f" ({skipped} пропущено по реестру)" if skipped else ""
                print(f"     {C.G}✓{C.N} +{len(new_all)} из {src_name}{suffix}. Всего по комбинации: {len(all_records)}/{limit}")
            except Exception as e:
...
def main():
    ...
    dedup_set = registry_dedup_set(registry)
    dk_this_run = set()
    print(f" {C.D}   Загружено {len(registry['records'])} записей из {REGISTRY_FILE}{C.N}\n")
    ...
    if night:
        print(f" {C.D}     Ночной режим — паузы ×1.3.{C.N}")
    print(f" {C.D}     {C.Y}Ctrl+C{C.N} — сохранить данные и выйти.{C.N}")
    print(f" {C.G}{'─'*55}{C.N}\n")
    ...
    try:
        ...
        for city, sphere in combos:
            ...
            records = collect_city_sphere(page, city, sphere, per_combo, seen_keys, night, registry, dedup_set)
            ...
            for rec in records:
                ...
                dk = make_dedup_key(rec["phones"])
                dk_this_run.add(dk)
                entry = registry["records"].get(dk)
                if entry:
                    if fname not in entry.setdefault("files", []):
                        entry["files"].append(fname)
                    entry["last_seen"] = datetime.now().strftime("%d.%m.%Y %H:%M")
            dk_this_run.clear()
        ...
        # ── Добивка: если недобрали до лимита ──
        if len(all_records) < total_limit:
            print(f"\n {C.Y}⚠  Недобор: {total_limit - len(all_records)} зап. Пытаюсь добрать...{C.N}")
            prev_total = -1
            while len(all_records) < total_limit:
                if prev_total == len(all_records):
                    break
                ...
                for city, sphere in combos:
                    ...
                    if (city, sphere) in skip_combos:
                        continue
                    need = total_limit - len(all_records)
                    print(f"\n {C.Y}⏳  Добивка: {city} / {sphere} (нужно ещё {need}){C.N}")
                    extra = collect_city_sphere(
                        page, city, sphere, need, seen_keys, night, registry, dedup_set
                    )
                    for rec in extra:
                        all_records.append(rec)
                        pbar.update(1)
                        ...
                        if entry:
                            ...
                            entry["last_seen"] = datetime.now().strftime("%d.%m.%Y %H:%M")
                    dk_this_run.clear()
                    print(f"     {C.Y}+{len(extra)} из {city} / {sphere}{C.N}")
                    if len(extra) == 0:
                        delay(night, 1, 2)
            save_registry(registry)
            print(f" {C.G}✅  Добивка завершена. Итого: {len(all_records)}/{total_limit}{C.N}\n")
        else:
            print()
        ...
        interrupted = True
        print(f"\n\n {C.Y}⏹  Получен Ctrl+C. Сохраняю данные...{C.N}")
        if ctx:
            try:
                ctx.close()
            except:
    ...
    if all_records:
        ...
        print(f"   {C.G}✅{C.N} Реестр: {C.BD}{os.path.abspath(REGISTRY_FILE)}{C.N}   ({len(registry['records'])} записей)")
